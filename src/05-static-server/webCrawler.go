	...
	for {
		...
		if token.DataAtom == atom.A {
			...
			case html.EndTagToken:
				if start == nil {
					fmt.Println("Link End found withour Srart:%s", text)
					continue
				}
...
func returnDownloader(url string,depth int){
	...
	for _,link := range links{
		fmt.Println(link)
		if depth +1 < MaxDepth{
			returnDownloader(link.url,depth+1)
		}
	}
}
